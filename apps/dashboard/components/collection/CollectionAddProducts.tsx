...
interface CollectionAddProductsProps {
	collectionId: number;
	existingProducts: CreateCollectionProductDto[];
	onClose: () => void;
}

export function CollectionAddProducts(props: CollectionAddProductsProps) {
	const { data: products } = useProducts({});
	const addProductToCollectionMutation = useAddProductToCollectionMutation();
	const [selectedIds, setSelectedIds] = useState<number[]>([]);
	const existingIds = useMemo(() => {
		return props.existingProducts.map(e => e.productId);
	...
	async function onSave() {
		const temp: CreateCollectionProductDto[] = selectedIds.map(id => ({
			productId: id,
			sortOrder: 0.,
		}));

		await addProductToCollectionMutation.mutateAsync({
			collectionId: props.collectionId,
			createCollectionProductDto: temp,
		});
		props.onClose();
	}
